...
#define IDC_VOICE_CARD 5300
// CVoiceRecorderDlg 對話方塊
...
IMPLEMENT_DYNAMIC(CVoiceRecorderDlg, CDialogEx)

CVoiceRecorderDlg::CVoiceRecorderDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_CVoiceRecorderDlg, pParent)
{

}

CVoiceRecorderDlg::~CVoiceRecorderDlg()
{
}

void CVoiceRecorderDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBO_PORTS_SELECT, m_ComboListPortsSelect);
	DDX_Control(pDX, IDC_EDIT_ALIAS, m_EditMachineAlias);
	DDX_Control(pDX, IDC_COMBO_VOICE_CARD_OUTPORTS_ASSIGN, m_AssignOutPortList);
}


BEGIN_MESSAGE_MAP(CVoiceRecorderDlg, CDialogEx)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDOK, &CVoiceRecorderDlg::OnBnClickedOk)
	ON_NOTIFY(NM_CUSTOMDRAW, 1000, &CVoiceRecorderDlg::OnColorSubProgramList)
	ON_NOTIFY(NM_DBLCLK, 1000, &CVoiceRecorderDlg::OnEditSubProgramListItem)
	ON_NOTIFY(NM_CLICK, 1000, &CVoiceRecorderDlg::OnClickSubProgramListItem)
	ON_CBN_SELCHANGE(IDC_COMBO_VOICE_CARD_OUTPORTS_ASSIGN, &CVoiceRecorderDlg::OnCbnSelchangeComboPotportsAssign)
	ON_CBN_SELCHANGE(IDC_COMBO_PORTS_SELECT, &CVoiceRecorderDlg::OnCbnSelchangeComboPortsSelect)
END_MESSAGE_MAP()


// CVoiceRecorderDlg 訊息處理常式


BOOL CVoiceRecorderDlg::OnInitDialog()
{
	...
	m_SubProgramListTitle.Create("語音卡類型的子機連線列表", WS_CHILD | WS_VISIBLE | SS_CENTER,
	...
	for (int i = 0; i < VIRTUAL_PORT_NUMS; i++)
	{
		...
		pOutLineEdit->DlgCtrlID = IDC_VOICE_CARD + i;
	...
	if (m_pEditOutLineTitle == NULL)
	{
		...
		m_pEditOutLineTitle->Create("分配語音卡的實體埠到虛擬外線埠", WS_CHILD | WS_VISIBLE | SS_CENTER,
			CRect(0, 0, 0, 0), this, 2000);
	}
...
int CVoiceRecorderDlg::ShowSubProgramList()
{
	// TODO: 請在此新增您的實作程式碼.
	...
	for (int i = 0; i < m_MachineNums; i++)
	{
		...
		lvI.pszText = "1";
		lvI.cchTextMax = 60;
		PSubProgramGroup_T pSubProgramGroup = &m_VoiceCardSubProgramGroup[i];
...
HBRUSH CVoiceRecorderDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (ThisCtrlID >= IDC_VOICE_CARD && ThisCtrlID <= IDC_VOICE_CARD + VIRTUAL_PORT_NUMS)
	{
		for (int i = 0; i < VIRTUAL_PORT_NUMS; i++)
		{
...
void CVoiceRecorderDlg::OnBnClickedOk()
{
...
BOOL CVoiceRecorderDlg::PreTranslateMessage(MSG* pMsg)
{
		...
		)
	{
		...
		if (pThisWnd == pEditWnd)
		{
			...
			PSubProgramGroup_T pSubProgramGroup = &m_VoiceCardSubProgramGroup[m_Item];
			if (strcmp(pSubProgramGroup->Alias, ThisAlias) != 0)
			{
				if (m_DatabaseAccessURL.SetMachineAliasByMachineID(MACHINE_TYPE_VOICE_CARD, m_Item + 1, ThisAlias, NULL) < 0)
				{
...
void CVoiceRecorderDlg::OnColorSubProgramList(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	else if (CDDS_ITEMPREPAINT | CDDS_SUBITEM)
	{
		...
		pSubProgramGroup = &m_VoiceCardSubProgramGroup[SubProgramIdex];
...
void CVoiceRecorderDlg::OnEditSubProgramListItem(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (m_Subitem == ID_LIST_SUB_PROGRAM_OUT_PORTS)
	{
		...
		else if (m_CallerIDSubProgramGroup[m_Item].OutPortNum > 0 || m_CallerIDSubProgramGroup[m_Item].ExtPortNum > 0)
			OccupyingType = MACHINE_TYPE_CALLER_ID_BOX;

		if (OccupyingType != 0)
		{
			CString TypeLabel = (OccupyingType == MACHINE_TYPE_PBX) ? _T("交換機") : _T("來電盒");
			CString Msg;
	...
	else if (m_Subitem == ID_LIST_SUB_PROGRAM_ALIAS)
	{
		...
		m_EditMachineAlias.SetWindowTextA(m_VoiceCardSubProgramGroup[m_Item].Alias);
...
int CVoiceRecorderDlg::LoadMachineData()
{
...
int CVoiceRecorderDlg::SetOutLineFont()
{
	// TODO: 請在此新增您的實作程式碼.
	double Ratio;
	int Index = 0;
...
int CVoiceRecorderDlg::ShowOutLineIcon()
{
	// TODO: 請在此新增您的實作程式碼.
	int Index = 0;
	POutPort_T pOutPort;
	...
	for (int i = 0; i < VIRTUAL_PORT_NUMS; i++)
	{
		...
		else
		{
			...
			int ThisOutPortNum = m_VoiceCardSubProgramGroup[MachineID - 1].OutPortNum;
			...
			if (ThisOutPortNum > 0)
			{
				if (pOutPort->MachineTypeCPhyPort > 0)
					sprintf_s(Buff, 100, "%03d", pOutPort->MachineTypeCPhyPort);
				else if (pOutPort->MachineTypeCPhyPort == 0)
					sprintf_s(Buff, 100, "");
...
void CVoiceRecorderDlg::OnDblClkOutPortListItem()
{
	// TODO: 在此加入控制項告知處理常式程式碼
	CWnd* pWnd = GetFocus();
	int CtrlID = pWnd->GetDlgCtrlID();

	if (CtrlID >= IDC_VOICE_CARD && CtrlID <= IDC_VOICE_CARD + VIRTUAL_PORT_NUMS)
	{
		m_Item = CtrlID - IDC_VOICE_CARD;
		int MachineID = m_OutPorts[m_Item].MachineID;
		int PBXOutPorts = m_PBXSubProgramGroup[MachineID - 1].OutPortNum;
		int ThisOutPorts = m_VoiceCardSubProgramGroup[MachineID - 1].OutPortNum;
		if (m_PrevCtrlID != CtrlID)
		{
			if (m_PrevCtrlID >= IDC_VOICE_CARD && m_PrevCtrlID <= IDC_VOICE_CARD + VIRTUAL_PORT_NUMS)
			{
...
void CVoiceRecorderDlg::OnLbdOutPortListItem()
{
	CWnd* pWnd = GetFocus();
	int CtrlID = pWnd->GetDlgCtrlID();
...
void CVoiceRecorderDlg::SetPortAssignList(int MachineID)
{
	CHAR Buff[200];
	int OutPortNum;
	int AssignedOutPort;
	...
	int PortIndex = 0;
	OutPortNum = m_VoiceCardSubProgramGroup[MachineID - 1].OutPortNum;
	AssignedOutPort = m_OutPorts[m_Item].MachineTypeCPhyPort;

	for (int i = 0; i < VIRTUAL_PORT_NUMS; i++)
	{
		if (
			m_OutPorts[i].MachineTypeCPhyPort > 0 &&
			m_OutPorts[i].IsInUsed &&
			m_OutPorts[i].MachineID == MachineID
			)
		{
			PortsArray[PortIndex] = m_OutPorts[i].MachineTypeCPhyPort;
			PortIndex++;
		}
	}
...
int CVoiceRecorderDlg::CheckOutVPortSetting(int ThisMachineIDIndex)
{
...
int CVoiceRecorderDlg::RedrawIcons()
{
	// TODO: 請在此新增您的實作程式碼.
	LoadMachineData();
...
void CVoiceRecorderDlg::OnClickSubProgramListItem(NMHDR* pNMHDR, LRESULT* pResult)
{
	m_ComboListPortsSelect.ShowWindow(FALSE);
}
...
void CVoiceRecorderDlg::OnCbnSelchangeComboPotportsAssign()
{
	// TODO: 在此加入控制項告知處理常式程式碼
	CEdit* pThisEdit = (CEdit*)GetDlgItem(m_OutPorts[m_Item].OutLineBtn.DlgCtrlID);
	int Ports;
	...
	Ret = m_DatabaseAccessURL.AssignPhyOutPortInfo(m_Item + 1, MACHINE_TYPE_VOICE_CARD, MachineID, Ports, NULL);
	if (Ret == 0)
	{
		if (m_DatabaseAccessURL.GetOutLines(m_OutPorts) < 0)
			m_OutLineNum = 0;
...
void CVoiceRecorderDlg::OnCbnSelchangeComboPortsSelect()
{
	// TODO: 在此加入控制項告知處理常式程式碼
	m_ComboListPortsSelect.ShowWindow(FALSE);
	int Ports;
	...
	case ID_LIST_SUB_PROGRAM_OUT_PORTS:
		...
		else if (CallerIDBoxOutPortNum != 0)
		{
			Len = sprintf_s(Buff, 200, "來電盒類型的外線數量已設定為%d\r\n", CallerIDBoxOutPortNum);
			Len += sprintf_s(Buff + Len, 200 - Len, "如欲設定語音卡的外線數量,請先將來電盒類型的外線數量設成0");
		...
		else
		{
			Ret = 0;
			Len = 0;
			if (PBXOutPortNum == 0 && VoiceCardOutPortNum != Ports)
			{
			...
			if (Ret == 0)
			{
				...
				if (Ret == 0)
				{
					...
					Ret = m_DatabaseAccessURL.SetMachineOutPortsByMachineID(MACHINE_TYPE_VOICE_CARD, SubProgramID, Ports, ErrMsg, &FailedPortCount);
					if (Ret == 0)
					{
						m_VoiceCardSubProgramGroup[m_Item].OutPortNum = Ports;
						ShowSubProgramList();
					}
					else
					{
...
void CVoiceRecorderDlg::MachineLogin(int MachineID, PCHAR pIPAddr, int SWVer)
{
	...
	m_DatabaseAccessURL.SetMachineConnected(MACHINE_TYPE_VOICE_CARD, MachineID, pIPAddr, TRUE, NULL);
}

void CVoiceRecorderDlg::MachineLogout(int MachineID)
{
	int Index = MachineID - 1;
	...
	m_DatabaseAccessURL.SetMachineConnected(MACHINE_TYPE_VOICE_CARD, MachineID, NULL, FALSE, NULL);
}

int CVoiceRecorderDlg::InitPorts()
{
	// TODO: 請在此新增您的實作程式碼.
	LoadMachineData();
	if (m_DatabaseAccessURL.GetOutLines(m_OutPorts) < 0)
