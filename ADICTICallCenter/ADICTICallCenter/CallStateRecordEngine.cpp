...
static int SentBufferEvent(PPackageEventBuffer_T pPackageEventBuffer)
{
    ...
    if (pEventParams)
    {
        ...
        /*在資料庫找到對應的虛擬OutPort*/
        DatabaseAccessURL.GetOutVPort(pEventParams->MachineType, pEventParams->MachineID, pEventParams->PhyOutPort, &pEventParams->OutVPort, NULL);
        /*在資料庫找到分機碼所對應的虛擬Port*/
        ...
        if (IsError == FALSE)
        {
            ...
            case EVENT_ID_EXTLINE_TALKING:
                ...
                break;
            case EVENT_ID_VOICE_REC_FILE_UUID:
                if (pEventParams->PhyOutPort != 0 && pEventParams->ToExtNum != 0)
                {
