<?php
	...
	$TableName = 'Inbound';
	$sql="SELECT * FROM $TableName WHERE InboundTEL='$Telphone' ORDER BY [InboundDate]";
	...
	if($result)
	{
		...
		$jsondata['fields'] = array('[Date], [Total Time], [Direction]');
		for($x = 0; $x < $Counter; $x++)
		{
			...
			$jsondata['data'][$x] = array($date->format('Y-m-d') . ' ' .  $time->format('H:i'), $Dur, $InboundDirectionUni);
