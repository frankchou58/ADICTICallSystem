...
int SendExternalCallAnswer(int MachineType, int MachineID, UINT32 Timestamp, int OutPortNo, int FromExtNum, int RingTimes)
{
	...
	SendLen = sizeof(ExternalCallAnswerPackage_T);
	memset(&ExternalCallAnswerPackage, 0x00, SendLen);
	...
	ExternalCallAnswerPackage.ExtNum = FromExtNum;
	ExternalCallAnswerPackage.RingTimes = RingTimes;

	SendLen = send(SendCtrlSock, (char*)&ExternalCallAnswerPackage, SendLen, 0);
