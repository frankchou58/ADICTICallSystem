...
static void ThreadMachineConnect(LPVOID lParam)
{
	...
	while (1)
	{
		...
		if (Ret < 0)
		{
			...
			{
				pCADICTIPBXSubMachineDlg->ShowMessage("!!!!離線!!!!-請關閉程式，並檢查伺服器及網路正常後再重啟程式。謝謝");
			}
			break;
		}
...
static time_t ConvertDateToUTC(CTime* pToday)
{
	struct tm timeinfo;
	time_t UTC;
...
static BOOL CheckMachineParam(int MachineType, int MachineID)
{
	...
	if (MachineType != PACKAGE_MACHINE_TYPE_PBX)
	{
		pCADICTIPBXSubMachineDlg->ShowMessage("錯誤：子機類型錯誤，請聯絡奕誠科技。");
		pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "子機類型錯誤");
		Ret = FALSE;
	}
	...
	{
		pCADICTIPBXSubMachineDlg->ShowMessage("錯誤：子機編號錯誤，請聯絡奕誠科技。");
		pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "子機編號錯誤");
		Ret = FALSE;
	}
...
int StartMachineClient()
{
	...
	pCADICTIPBXSubMachineDlg->ShowMessage("與伺服器連線中，請稍後!!!!!");

	MachineServerIP = AfxGetApp()->GetProfileString("SystemSetting", "MachineServerIP", "");
	Ret = InitMachineClient(MachineServerIP.GetBuffer());
	if (Ret == -1)
	{
		pCADICTIPBXSubMachineDlg->ShowMessage("錯誤：子機網路初始錯誤。");
		pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "子機網路初始錯誤");
	}
	else if (Ret == -2)
	{
		pCADICTIPBXSubMachineDlg->ShowMessage("錯誤：子機連線失敗，請確定伺服器是否開啟或[子機伺服器網址]設定是否正確。");
		pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "子機連線失敗");
	}
	...
	if (Ret == 0)
	{
		...
		if (m_hThreadMachineProtoco != NULL)
		{
			...
			Ret = LoginMachineServer(1, MachineID, OutPortNum, ExtPortNum);
			CHAR Message[100];
			...
			if (Ret > 0)
			{
				if (m_MachineResponsePackage.ResponseID == PACKAGE_RESPONSE_LOGIN_OK)
				{
					...
					pViewLogDlg->WriteLog(LOG_LEVEL_MESSAGE, LOG_TYPE_SUB_MACHINE, "與伺服器連線成功");
					m_LoginSuccess = TRUE;
				}
				else if (m_MachineResponsePackage.ResponseID == PACKAGE_RESPONSE_LOGIN_ALREADY)
				{
					sprintf_s(Message, 100, "錯誤：機碼%d已登入。", MachineID);
					pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "機碼已登入");
				}
				else if (m_MachineResponsePackage.ResponseID == PACKAGE_RESPONSE_MACHINE_NOT_SETTING)
				{
					...
					pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "尚未設定外線數");
				}
				else if (m_MachineResponsePackage.ResponseID == PACKAGE_RESPONSE_MACHINE_OUTPORTS_NOTMATCH)
				{
					...
					pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "外線數量不符");
				}
				else if (m_MachineResponsePackage.ResponseID == PACKAGE_RESPONSE_MACHINE_EXTPORTS_NOTMATCH)
				{
					...
					pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "內線數量不符");
				}
				else if (m_MachineResponsePackage.ResponseID == PACKAGE_RESPONSE_OVER_MACHINE_CALLBLOCK_NUMS)
				{
					sprintf_s(Message, 100, "錯誤：超過子機數。");
					pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "超過子機數");
				}
			}
			else
			{
				sprintf_s(Message, 100, "登入失敗");
				pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "登入失敗");
			}
...
int ClientSendCallStatus(int CallType, PCallParams_T pCallParams)
{
	...
	{
		pCADICTIPBXSubMachineDlg->ShowMessage("錯誤：外線編號 > 該子機的外線數量設定，請檢查參數設定。");
		pViewLogDlg->WriteLog(LOG_LEVEL_ERROR, LOG_TYPE_SUB_MACHINE, "外線編號編號超過設定");
		return -1;
	}
	...
	pViewLogDlg->WriteLog(LOG_LEVEL_MESSAGE, LOG_TYPE_SUB_MACHINE, Message);
	...
	case PACKAGE_EVENT_TYPE_EXTERNAL_CALL_OUT:
		...
		break;
	case PACKAGE_EVENT_TYPE_EXTERNAL_CALL_ANSWER:
		sprintf_s(Message, 100, "通話：時間=%s 外線號碼=%d與分機號碼=%d,振鈴次數=%d次[通話中]", Date, pCallParams->OutPortNo, pCallParams->FromExtNo, pCallParams->RingNums);
		Ret = SendExternalCallAnswer(pCallParams->MachineType, pCallParams->MachineID, Timestamp, pCallParams->OutPortNo, pCallParams->FromExtNo, pCallParams->RingNums);
		break;
	case PACKAGE_EVENT_TYPE_EXTERNAL_SECOND_DIAL:
		break;
	case PACKAGE_EVENT_TYPE_EXTERNAL_CALL_HANGUP:
