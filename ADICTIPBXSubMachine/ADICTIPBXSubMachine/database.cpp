...
int InsertLogDB(int Timestame, int Level, int Type, PCHAR pMessage)
{
	...
	char ErrMessage[100];
	char* zErr;
	char SqlStatement[] = "INSERT INTO log (timestamp,level,type,message) VALUES ('%d','%d','%d','%s');";
	char SqlCommand[1000];
	int ret = 0;
	time_t rawtime;
	...
	sprintf_s(SqlCommand, SqlStatement, Timestame, Level, Type, pMessage);
...
int QueryLogDB(PLogParam_T pLogParam, PLog_T pLogDB)
{
	...
	Len = sprintf_s(SqlCommand, 1000, SqlStatement, pLogParam->StartTimestamp, pLogParam->EndTimestamp, pLogParam->Type);
	...
	Len += sprintf_s(SqlCommand + Len, 1000 - Len, " ORDER BY timestamp DESC");
	Numbers = 0;
