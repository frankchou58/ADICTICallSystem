...
void CSettingDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//  DDX_Control(pDX, IDC_STATIC_EXT_PORTS, m_ExtPortNum);
...
BOOL CSettingDlg::OnInitDialog()
{
	...
	m_EditOutPortNum.SetCurSel(OutPort - 1);
	RECT EditOutPortRect;
	EditOutPortRect.left = TitleOutPortNumRect.right;
	EditOutPortRect.top = TitleOutPortNumRect.top;
	EditOutPortRect.bottom = EditOutPortRect.top + m_ButtonFontHeight * 12;
	EditOutPortRect.right = EditOutPortRect.left + 100;
	m_EditOutPortNum.MoveWindow(&EditOutPortRect);
	m_EditOutPortNum.SetFont(m_pButtonFont);
...
void CSettingDlg::OnBnClickedOk()
{
	...
	if (Changed)
	{
		...
		// 舊連線）重新連線，不用再重啟整個程式。
		CADICTIPBXSubMachineDlg* pCADICTIPBXSubMachineDlg = (CADICTIPBXSubMachineDlg*)AfxGetApp()->m_pMainWnd;
		pCADICTIPBXSubMachineDlg->ShowMessage("設定已變更，正在重新連線伺服器...");
		StartMachineClient();
	}
